...
type BatchInterface interface {
	Worker(ctx context.Context, collectorSchema v2alpha1.CollectorSchema, opts *common.MirrorOptions) (v2alpha1.CollectorSchema, error)
}
...
func (o *ChannelConcurrentBatch) Worker(ctx context.Context, collectorSchema v2alpha1.CollectorSchema, opts *common.MirrorOptions) (v2alpha1.CollectorSchema, error) {
	startTime := time.Now()
	...
	p := mpb.New(mpb.PopCompletedMode(), mpb.ContainerOptional(mpb.WithOutput(io.Discard), !opts.IsTerminal()))
...
func logResults(log clog.PluggableLoggerInterface, copyMode string, copiedImages, collectorSchema *v2alpha1.CollectorSchema) {
	...
	logResult(log, copyMode, "release", copiedImages.TotalReleaseImages, collectorSchema.TotalReleaseImages)
	logResult(log, copyMode, "operator", copiedImages.TotalOperatorImages, collectorSchema.TotalOperatorImages)
	logResult(log, copyMode, "additional", copiedImages.TotalAdditionalImages, collectorSchema.TotalAdditionalImages)
	logResult(log, copyMode, "helm", copiedImages.TotalHelmImages, collectorSchema.TotalHelmImages)
}
...
func logImageSuccess(log clog.PluggableLoggerInterface, image *v2alpha1.CopyImageSchema, opts *common.MirrorOptions) {
	if opts.IsTerminal() {
...
func logImageError(log clog.PluggableLoggerInterface, image *v2alpha1.CopyImageSchema, opts *common.MirrorOptions) {
	if opts.IsTerminal() {
		// It'll be printed by the spinner
		return
	}

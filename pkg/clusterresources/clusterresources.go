...
import (
	...
	updateservicev1 "github.com/lmzuccarelli/golang-oc-mirror-refactor/pkg/clusterresources/updateservice/v1"
...
func (o *ClusterResourcesGenerator) CatalogSourceGenerator(allRelatedImages []v2alpha1.CopyImageSchema) error {
	...
	for _, copyImage := range allRelatedImages {
		...
		if copyImage.Type == v2alpha1.TypeOperatorCatalog && !strings.Contains(copyImage.Destination, o.Options.LocalStorageFQDN) {
			if firstCatalog {
...
func (o *ClusterResourcesGenerator) UpdateServiceGenerator(graphImageRef, releaseImageRef string) error {
	...
	osusPath := filepath.Join(o.Options.WorkingDir, clusterResourcesDir, updateServiceFilename)
...
func (o *ClusterResourcesGenerator) GenerateSignatureConfigMap(allRelatedImages []v2alpha1.CopyImageSchema) error {
	...
	sigDir := filepath.Join(o.Options.WorkingDir, signatureDir)
	...
	if len(cm.BinaryData) > 0 {
		crPath := filepath.Join(o.Options.WorkingDir, clusterResourcesDir)
